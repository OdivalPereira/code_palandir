...
import { FolderOpen, Sparkles, Layers, ArrowRight, FoldHorizontal, UnfoldHorizontal } from 'lucide-react';
  ...
  return (
    <div className="h-full w-full bg-slate-950">
      ...
      >
        ...
        {/* Floating Canvas Expansion & Depth Toolbar */}
        <Panel position="top-left" className="!m-3">
          <div className="flex items-center gap-1.5 rounded-xl border border-slate-800/90 bg-slate-950/85 p-1.5 shadow-2xl backdrop-blur-md text-xs">
            {/* Depth Level Selector */}
            <div className="flex items-center gap-0.5 bg-slate-900/90 p-0.5 rounded-lg border border-slate-800/80">
              <button
                type="button"
            ...
            <button
              ...
              className="px-2 py-1 rounded-lg border border-slate-800 bg-slate-900/90 text-slate-300 hover:bg-slate-800 hover:text-white transition-colors flex items-center gap-1 text-[11px] font-medium"
              title={expandedNodeIds.size > 0 ? 'Recolher todos os nós' : 'Expandir todos os nós'}
            >
              {expandedNodeIds.size > 0 ? (
                <>
                  <FoldHorizontal className="h-3 w-3 text-indigo-400" />
                  <span>Recolher</span>
                </>
              ) : (
                <>
                  <UnfoldHorizontal className="h-3 w-3 text-indigo-400" />
                  <span>Expandir</span>
                </>
              )}
            </button>

            {/* Visible Node Count Badge */}
            <span className="text-[10px] font-mono text-slate-400 px-1.5 py-0.5 rounded bg-slate-900/50 border border-slate-800/40">
              {nodes.length} nós
            </span>
          </div>
        </Panel>
